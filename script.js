...
async function judgeVoteResult() {
  ...
  // 全プレイヤーの得票数を0票で初期化し、名前とIDを紐付けます
  currentPlayers.forEach((player) => {
    counts[player.name] = 0;
    voteData[player.name] = player.id;
  });

  votesSnapshot.docs.forEach((voteDoc) => {
    const vote = voteDoc.data();
    if (counts[vote.targetName] !== undefined) {
      counts[vote.targetName] += 1;
    } else {
      counts[vote.targetName] = 1;
      voteData[vote.targetName] = vote.targetUid;
    }
  });
